...
if __name__ == "__main__":
    ...
    if args.from_resume == 0:
        ...
        sub_dir_name = f"{timestamp}_sft_{freeze_status}"
    ...
    # 加载 Pretrain 检查点（如果有指定）
    if args.from_resume == 1 and ckp_data:
        model.load_state_dict(ckp_data['model'], strict=False)
        Logger("Loaded SFT checkpoint for resume")
    elif args.use_pretrain_model == 1:
        # 自动加载最新的 Pretrain 模型
        from trainer_utils import find_latest_checkpoint
        latest_dir, _ = find_latest_checkpoint(base_dir, stage_prefix="pretrain")
        if latest_dir:
            pretrain_ckp_path = os.path.join(latest_dir, "pretrain_vlm_checkpoint.pt")
            if os.path.exists(pretrain_ckp_path):
                pretrain_ckp = torch.load(pretrain_ckp_path, map_location='cpu')
                model.load_state_dict(pretrain_ckp['model'], strict=False)
                Logger(f"Loaded Pretrain checkpoint from: {latest_dir}")
            else:
                Logger(f"Warning: Pretrain checkpoint not found at {pretrain_ckp_path}")
        else:
            Logger("Warning: No Pretrain checkpoint directory found")
    elif args.pretrain_checkpoint and os.path.exists(args.pretrain_checkpoint):
        pretrain_ckp = torch.load(args.pretrain_checkpoint, map_location='cpu')
        model.load_state_dict(pretrain_ckp['model'], strict=False)
        Logger(f"Loaded Pretrain checkpoint from: {args.pretrain_checkpoint}")
    ...
    with TrainingProgressBar(total_steps, desc="MiniQ-VL SFT", log_interval=args.log_interval) as pbar:
